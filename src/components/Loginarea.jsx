...
function Loginarea(props) {
    ...
    return (<div className="login-form " >
        <form onSubmit={formik.handleSubmit}>
            <input type="text" id="username" name="username" placeholder="Username" className="username" autoComplete="off" onChange={formik.handleChange} value={formik.values.username} /><br />
            {formik.touched.username ? (formik.errors.username ? <p className="pwd-yup">{formik.errors.username}</p> : null) : null}
            <div className="username"><input type={type} id="password" name="password" placeholder="password" autoComplete="off" className="password-area" onChange={formik.handleChange} value={formik.values.Password} /><span onClick={handleToggle} style={{ color: '#00c0f5' }}><Icon icon={icon} size={20} /></span></div><br />
